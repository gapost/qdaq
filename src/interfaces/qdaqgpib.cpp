...
#endif
#else
#error "Unsupported platform: GPIB interface requires Linux or Windows"
#endif

QDaqGpib::QDaqGpib(const QString &name) : QDaqInterface(name)
{
    // 32 empty places
    ports_.fill(0, 32);
}
...
int QDaqGpib::write(uint port, const char *buff, int len, int e)
{
    ...
    if (ThreadIbsta() & ERR)
    {
        pushGpibError(
            ThreadIberr(),
            QString("Send(%1,%2,%3,%4,%5)").arg(address()).arg(port).arg(QString(QByteArray(buff, len))).arg(len).arg(eot));
        return 0;
    }
    else
        return len; // ibcntl; linux-gpib sends +1 byte
}
...
QDaqVector QDaqGpib::findListeners()
{
    QMutexLocker L(&comm_lock);
