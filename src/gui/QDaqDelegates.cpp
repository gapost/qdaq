...
/**************************************************************************************************/
DisplayDelegate::DisplayDelegate(QWidget *w, QDaqChannel *ch) : QObject(w),
																widget_(w),
																channel_(ch)
{
...
void DisplayDelegate::updateDisplay()
{
	...
	if (widget_.isText())
		widget_.write(channel_->formatedValue());
	else
		widget_.write(channel_->value());
}
...
{
	if (property_.isWritable() && v.isValid())
		property_.write(obj_, v);
}
