...
QDaqConsole::QDaqConsole(QDaqSession *s, QWidget *parent) : QConsoleWidget(parent), session_(s)
{
    ...
    if (session_->index())
        setWindowTitle(QString("Console #%1").arg(session_->index()));
    else
        setWindowTitle(QString("Console #0 - Root"));
...
void QDaqConsoleTabWidget::addConsole()
{
    ...
    if (count() == 0)
    {
        ...
        child = new QDaqConsole(QDaqObject::root()->newSession());
