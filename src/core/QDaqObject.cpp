...
QDaqObject::QDaqObject(const QString &name) : QObject(0), comm_lock(QMutex::Recursive)
{
    setObjectName(name);
...
{
    if (this == root())
        return true; // root is always atached
    QDaqObject *p = parent();
    while (p && p != root())
        p = p->parent();
    return p;
...
void listPropertiesHelper(const QDaqObject *m_object, QString &S, const QMetaObject *metaObject, int &level)
{
    ...
    for (int idx = metaObject->propertyOffset(); idx < metaObject->propertyCount(); idx++)
    {
        ...
        else if (metaProperty.isEnumType())
        {
            ...
            P += metaProperty.read(m_object).toString();
        P += '\n';
    }
...
QDaqObject *QDaqObject::insertBefore(QDaqObject *newobj, QDaqObject *existingobj)
{
    ...
    if (!checkName(newobj->objectName()))
        return 0;
