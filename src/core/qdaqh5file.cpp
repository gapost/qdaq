...
bool QDaqH5File::isQDaq(const QString &fname)
{
    ...
    try
    {
        ...
        if (!ret || fileType != "QDaq")
            return false;

        if (!root.read("FileVersionMajor", vMajor) || !root.read("FileVersionMinor", vMinor))
            return false;
...
void QDaqH5File::writeRecursive(const QH5Group &h5g, const QDaqObject *obj)
{
    ...
    foreach (const QDaqObject *ch, obj->children())
        writeRecursive(objGroup, ch);
}
...
void QDaqH5File::readRecursive(const QH5Group &h5g, QDaqObject *&parent_obj)
{
    ...
    foreach (QByteArray groupName, groups)
    {
        ...
        {
            if (className == "QDaqRoot")
                className = "QDaqObject";
            className += "*";
            int id = QMetaType::type(className.toLatin1());
            if (id != 0)
            { // TODO: report to user if the type is not found
...
void QDaqH5File::newHelper(Version v)
{
    ...
    {
    case V_1_0:
        helper_ = new h5helper_v1_0(this);
        return;
    case V_1_1:
        helper_ = new h5helper_v1_1(this);
        return;
    default:
        helper_ = new h5helper_v1_1(this);
        return;
