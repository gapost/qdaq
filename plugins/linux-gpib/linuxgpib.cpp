...
QDaqLinuxGpib::~QDaqLinuxGpib()
{
}

void QDaqLinuxGpib::pushGpibError(int code, const QString& comm)
{
...
bool QDaqLinuxGpib::open_port(uint id, QDaqDevice* dev)
{
    QMutexLocker L(&comm_lock);
...
void QDaqLinuxGpib::close_port(uint id)
{
    QMutexLocker L(&comm_lock);
    Addr4882_t addr[2] = {(Addr4882_t)id, NOADDR};
...
void QDaqLinuxGpib::clear_port(uint id)
{
    QMutexLocker L(&comm_lock);
...
int QDaqLinuxGpib::read(uint port, char* data, int len, int eoi)
{
    QMutexLocker L(&comm_lock);
...
int QDaqLinuxGpib::readStatusByte(uint port)
{
    QMutexLocker L(&comm_lock);
    short ret(0);
...
int QDaqLinuxGpib::write(uint port, const char* buff, int len, int e)
{
    QMutexLocker L(&comm_lock);
    int eot =  (e & 0x0000FF00) >> 8;
    ::Send(address(),(Addr4882_t)port, buff, len, eot);
    if (ThreadIbsta() & ERR)
    {
        pushGpibError(
            ThreadIberr(),
...
void QDaqLinuxGpib::trigger(uint id)
{
     QMutexLocker L(&comm_lock);
...
void QDaqLinuxGpib::setTimeout_(uint ms)
{
