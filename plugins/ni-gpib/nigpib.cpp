...
int QDaqNiGpibPlugin::error()
{
    return ThreadIberr();
}
int QDaqNiGpibPlugin::count()
{
    return ThreadIbcnt();
}

bool QDaqNiGpibPlugin::hasError()
{
    return status() & ERR;
}

void QDaqNiGpibPlugin::setTimeout(int boardID, int ms)
{
...
void QDaqNiGpibPlugin::SendIFC(int boardID)
{
    ::SendIFC(boardID);
}
void QDaqNiGpibPlugin::EnableRemote(int boardID, int id)
{
    Addr4882_t addr[2] = {(Addr4882_t)id, NOADDR};
    ::EnableRemote(boardID,addr);
}
void QDaqNiGpibPlugin::EnableLocal(int boardID, int id)
{
    Addr4882_t addr[2] = {(Addr4882_t)id, NOADDR};
    ::EnableLocal(boardID,addr);
}
void QDaqNiGpibPlugin::QDaqNiGpibPlugin::DeviceClear(int boardID, int address)
{
    ::DevClear(boardID,(Addr4882_t)address);
}
void QDaqNiGpibPlugin::Receive(int boardID, int address, char* data, int len, int eoi)
{
...
void QDaqNiGpibPlugin::FindListeners(int boardID, const QVector<int>& addresses, QVector<int>& results)
{
...
const char* QDaqNiGpibPlugin::errorMsg(int error_code)
{
